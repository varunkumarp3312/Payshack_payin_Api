...
public class PayinEndToEndTest extends BaseTest {
	...
	public void tc01_verifyInitiateAndStatusFlow() throws Exception {
		...
		InitiateIntentRequest request = new InitiateIntentRequest().setOrderId(orderId).setAmount("100")
				.setFirstName("pavan").setLastName("kumar").setEmail("pmkirru33@gmail.com").setPhone("8970411423")
				.setUserIP("10.210.10.1").setUserId("dddd");
		...
		Assert.assertFalse("txnRefId should not be empty", initiateData.path("txnRefId").asText().trim().isEmpty());
		Assert.assertFalse("paymentUrl should not be empty", initiateData.path("paymentUrl").asText().trim().isEmpty());
		...
		log.info("Initiated | orderId={} | txnRefId={}", TestContext.getOrderId(), TestContext.getTxnRefId());
		...
		log.info("Waiting 5s before status check...");
		Thread.sleep(5_000);
		...
		Assert.assertFalse("txnRefId should not be empty", statusData.path("txnRefId").asText().trim().isEmpty());

//        Assert.assertFalse(
//                "utr should not be empty",
//                statusData.path("utr").asText().trim().isEmpty());

		Assert.assertFalse("amount should not be empty", statusData.path("amount").asText().trim().isEmpty());
		...
		log.info("Status={} | TxnRefId={} | UTR={} | Amount={}", TestContext.getTransactionStatus(),
				TestContext.getTxnRefId(), TestContext.getUtr(), TestContext.getAmount());
	}
