...
public class PayinTelegramFlowTest extends BaseTest {
    ...
    private static final long PAID_BUTTON_TIMEOUT_SECONDS   = 300;
        ...
        try {
            while (true) {
                ...
                if (callback == null) {
                    ...
                    Assert.fail("Timed out waiting for button click.");
                }
                ...
                if (callback.isNotNow) {
                    ...
                    if (minutes == null) {
                        ...
                        Assert.fail("No minutes input received within " + MINUTES_REPLY_TIMEOUT_SECONDS + "s.");
                    }
                ...
                if (!"SUCCESS".equalsIgnoreCase(finalStatus)) {
                    ...
                    log.error("Test FAILED | orderId={} | status={}", orderId, finalStatus);
                    Assert.fail("Payment failed. Final status: " + finalStatus);
                }
                ...
                if (payload != null) {
                    telegram.editMessageText(messageId,
                            msgCallbackReceived(orderId, txnRefId, finalStatus, callback.fromUser, payload));
                    log.info("Test PASSED + callback received | orderId={}", orderId);
                } else {
                    telegram.editMessageText(messageId,
                            msgCallbackNotReceived(orderId, txnRefId, finalStatus, callback.fromUser));
                    log.warn("SUCCESS but no callback received | orderId={}", orderId);
                }
                break;
            }
    ...
    private String runStatusCheckWithRetry(TelegramNotifier telegram, String orderId,
        ...
        if (!"INITIATED".equalsIgnoreCase(status)) return status;

        for (int attempt = 1; attempt <= STATUS_RETRY_COUNT; attempt++) {
            telegram.editMessageText(messageId, msgInitiated(orderId, attempt));
            log.info("INITIATED. Waiting {}s before retry {}/{}...",
                    STATUS_RETRY_INTERVAL_MS / 1_000, attempt, STATUS_RETRY_COUNT);
    ...
    private String msgCallbackReceived(String orderId, String txnRefId,
                                       String status, String by, String payload) {
        String display = payload.length() > 800 ? payload.substring(0, 800) + "..." : payload;
